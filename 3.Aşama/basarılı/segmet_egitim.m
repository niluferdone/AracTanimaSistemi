dataDir = fullfile('C:\Users\Mehmet Akif YILMAZ\Desktop\segment\road_line');%içerisinde fotoðraf var bu klasörün içinde eðitm ve test diye klasör var. eðitim için hem segmetlenmiþ fotoðraflar 
dataDi = fullfile(dataDir,'egitim');
dataD = fullfile(dataDi,'egit');
imDir = fullfile(dataD,'segmentlenmemis');
pxDir = fullfile(dataD,'segmentlenmis');
imds = imageDatastore(imDir);
I = readimage(imds,18);
%figure
%imshow(I)
classNames = ["arac","arka_plan"];
...
numClasses = 2;   %"yol" "arka_plan" "isaret"  olmak üzere piksel ýd'li 3 adet sýnýf var
